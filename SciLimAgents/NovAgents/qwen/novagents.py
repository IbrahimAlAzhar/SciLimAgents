...
OUTPUT_DIR = "llm_agents/qwen/novagents"
os.makedirs(OUTPUT_DIR, exist_ok=True)
OUTPUT_CSV = os.path.join(OUTPUT_DIR, "df_qwen_3b_novelty_agents_output.csv")
...
def tok_len(text: str) -> int:
    return len(_tiktoken_enc.encode(text or "", disallowed_special=()))

def truncate_to_tokens(text: str, max_tokens: int, keep: str = "head") -> str:
    if not text:
        return ""
    # ids = _tiktoken_enc.encode(text) 
    ids = _tiktoken_enc.encode(text, disallowed_special=())
...
def call_llm(system_prompt: str, user_message: str,
    ...
    """Run a single inference call on the locally-loaded Qwen model."""
    messages = [
...
def run_pipeline():
    ...
    for i in tqdm(range(len(df)), desc="Qwen-3B Novelty Analysis"):
        row = df.iloc[i]
