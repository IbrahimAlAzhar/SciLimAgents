...
# ── Configuration ────────────────────────────────────────────────────
DF_PATH = "llm_agents/gpt/merger_lim_nov/df_lim_nov_merge_200.csv"

# DF_NOV_PATH = "llm_agents/llama_3_8B_new/novagents/df_llm_novagents_llama3_8b_seq_output.csv"

# Output Directory and Files
OUTPUT_DIR = "llm_agents/gpt/merger_lim_nov/merge_latest"
...
def main():
    ...
    # df_nov = pd.read_csv(DF_PATH)

    # Ensure IDs exist
    if 'id' not in df.columns or 'id' not in df.columns:
        raise ValueError("Both dataframes must contain an 'id' column.")

    print(f"Initial lengths - df_lim: {len(df)}, df_nov: {len(df)}")

    # 1. Identify IDs with "SKIPPED_SHORT_TEXT"
    lim_skipped_mask = df['final_merged_limitations'].astype(str).str.contains("SKIPPED_SHORT_TEXT", na=False)
    nov_skipped_mask = df['novelty_report'].astype(str).str.contains("SKIPPED_SHORT_TEXT", na=False)

    lim_skipped_ids = set(df[lim_skipped_mask]['id'])
    nov_skipped_ids = set(df[nov_skipped_mask]['id'])
    all_skipped_ids = lim_skipped_ids.union(nov_skipped_ids)

    # 2. Find common IDs that should NOT be skipped
    common_ids = set(df['id']).intersection(set(df['id']))
    ...
    nov_dict = df.set_index('id')['novelty_report'].to_dict()
