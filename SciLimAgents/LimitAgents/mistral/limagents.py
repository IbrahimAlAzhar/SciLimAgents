...
OUTPUT_DIR = "llm_agents/mistral_new/limagents/part_1"
OUTPUT_FILE = os.path.join(OUTPUT_DIR, "df_mistral_7b_v3_7_agents_novelty_output.csv")
...
# Mistral 7B Instruct v0.3 (HF id + local cache dir)
...
# ── Context budget (Mistral 7B Instruct v0.3: 32k context window) ───────────
...
# Hard guard inside call_llm
MODEL_MAX_INPUT_TOKENS = MAX_PROMPT_TOKENS

# Token budgets for paper / citations inside prompts
...
# Must satisfy: MAX_PROMPT_TOKENS + MAX_NEW_TOKENS <= max_model_len <= 32768.
VLLM_MAX_MODEL_LEN = SAFE_INPUT_LIMIT + MAX_NEW_TOKENS + 256   # = 28956
VLLM_GPU_MEM_UTIL = 0.90       # Mistral 7B needs more headroom than a 3B model
VLLM_DTYPE = "float16"
VLLM_TENSOR_PARALLEL = 1

# Save frequency
SAVE_EVERY = 5
SLEEP_SEC = 0.0   # vLLM is local — no rate limit, no need to sleep

# ============================================================
# 2) LOAD MISTRAL 7B INSTRUCT v0.3 VIA vLLM
# ============================================================

print("Loading Mistral 7B Instruct v0.3 tokenizer + vLLM engine...")
...
# Pre-compute Mistral stop token ids.
# Mistral's instruct template uses [INST] ... [/INST] with </s> as EOS.
# There is no <|im_end|>-style sentinel, so EOS alone is sufficient.
...
def _build_mistral_messages(system_prompt: Optional[str], user_prompt: str) -> List[Dict[str, str]]:
    """
    Mistral Instruct v0.3's chat template does NOT support a separate 'system' role.
    Prepend any system content into the first user turn instead.
    """
    if system_prompt:
...
def truncate_user_prompt_for_context(system_prompt: Optional[str], user_prompt: str) -> str:
    ...
    Adapted for Mistral v0.3 (no system role; system text is folded into user).
    """
    msgs = _build_mistral_messages(system_prompt, user_prompt)
    ...
    while low <= high:
        ...
        trial_msgs = _build_mistral_messages(system_prompt, trial_prompt)
...
def summarize_relevant_papers_list(relevant_list: List[str], max_items: int = 3) -> str:
    """
    Summarize each item from relevant_papers_list using Mistral,
    ...
    for j, item in enumerate(take, start=1):
        ...
        # Generous input budget for summarization calls (Mistral has 32k window)
        item_tr = truncate_to_tokens(item_clean, max_tokens=8000)
        sp = summarization_prompt_for_relevant_paper(item_tr, j)
        out = mistral_generate(sp, system_prompt="You are an expert scientific summarizer.", max_new_tokens=500)
...
def call_llm(
    ...
    """Run a single inference call through the local vLLM engine (Mistral 7B v0.3)."""
    # Mistral has no 'system' role — merge into the user turn.
    messages = _build_mistral_messages(system_prompt or None, user_message)
...
def mistral_generate(
    ...
    """
    Thin wrapper that runs the binary-search context-fitting step, then dispatches
    to vLLM via call_llm. Mirrors the qwen_generate / llama_generate interface.
    """
...
def run_pipeline():
    ...
    print(
        f"Starting sequential 7-agent generation (Mistral 7B Instruct v0.3) "
