...
def parse_merged_limitations(text_str):
    ...
    for line in lines:
        ...
        # 2. NEW: Check for Asterisk Category Headers (e.g., "**Explicit Limitations:**")
        cat_match = re.match(r'^\*\*(.*?)\*\*:?$', line)
        if cat_match:
            # Extracts the text inside the ** ** and strips any trailing colons
            current_category = cat_match.group(1).strip()
            continue

        # 3. Check for Limitations using the double asterisk split with numbers
        # Matches format: "1. **Limitation Title**: Limitation body..."
        if '**' in line and line[0].isdigit():
            parts = line.split('**')
            
            if len(parts) >= 3:
                title = parts[1].strip()
                body = '**'.join(parts[2:]).lstrip(': ').strip()
        ...
        # 4. Fallback for standard bullet points (Handles both '-' and '*')
            ...
            # Check if this bullet point has double asterisks inside it
            if '**' in body:
                parts = body.split('**')
                if len(parts) >= 3:
                    title = parts[1].strip()
            ...
            # NEW: Check if the bullet uses a colon to separate Title and Description
            # Matches format: "* Assuming a fixed meaning of words: The study assumes..."
            elif ':' in body:
                parts = body.split(':', 1) # Split only on the first colon
                title = parts[0].strip()
                desc = parts[1].strip()
                full_limitation = f"{title}: {desc} (- **{current_category}**)"
