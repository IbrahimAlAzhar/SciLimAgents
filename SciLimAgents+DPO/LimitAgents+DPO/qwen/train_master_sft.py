"""
train_master_sft.py — SFT training for the Master agent (LoRA on Qwen 2.5 3B).
...
SFT_DATA     = "other_experiments/dpo/sft_and_dpo_pairs/sft_dataset_master_long.json"
OUTPUT_DIR   = "other_experiments/dpo/train/master_sft"
...
def load_sft_data(path: str) -> Dataset:
    ...
    The JSON has a 'messages' field in chat format.

    NOTE: Master samples may be fewer than worker/leader since there is
    only 1 master turn per rollout (vs 12 worker + 7 leader turns).
    Consider increasing NUM_EPOCHS if dataset is small.
    """
    with open(path) as f:
        records = json.load(f)
    ...
    if len(processed) < 50:
        print(f"[WARN] Only {len(processed)} master samples. "
              "Consider increasing NUM_EPOCHS to 5-8 for better convergence, "
              "or re-run reward_model.py with higher SFT_TOP_K_BY_ROLE for master.")
