import pandas as pd 
df = pd.read_csv("agentreview/llama_qwen_mistral/llama_run/summary.csv")  
len(df)  
...
def parse_merged_limitations(text_str):
    ...
    # 1. Handle literal '\n' strings if the LLM output escaped them
    text_str = text_str.replace('\\n', '\n')

    limitations = []
    lim_id = 1 # Starting at 1 to match the source text numbering

    # 2. Split the text using newline followed by digits and a dot (e.g., "\n1. ", "\n2. ")
    # (?:^|\n) ensures it matches either the start of the string or a newline.
    parts = re.split(r'(?:^|\n)\d+\.\s+', text_str)
    
    for part in parts:
        part = part.strip()
        
        # Skip empty segments or the introductory "Summary:" block
        if not part or part.startswith('Summary:'):
            continue

        # 3. Check for bracketed categories at the start of the block (e.g., "[data / major]")
        # flags=re.DOTALL allows the regex to match across multiple lines within the split part
        bracket_match = re.match(r'^\[(.*?)\]\s+(.*)', part, flags=re.DOTALL)
        
        if bracket_match:
            current_category = bracket_match.group(1).strip()
            body = bracket_match.group(2).strip()
        else:
            current_category = "General"
            body = part

        # 4. Clean up the "evidence:" line by collapsing it into the same string
        # This turns "\n   evidence: Text" into " | Evidence: Text" for cleaner dataframe storage
        body_clean = re.sub(r'\s*\n\s*evidence:\s*', ' | Evidence: ', body, flags=re.IGNORECASE)
        
        # Format the final extracted string
        full_limitation = f"{body_clean} (- **{current_category}**)"
...
df['mistral_limitations_list'] = df['final_limitations_text'].apply(parse_merged_limitations) 
...
for i, (index, row) in enumerate(tqdm(df.iterrows(), total=len(df), desc="Processing Rows")):
    ...
    print("i is",i) 
    if (i + 1) % 10 == 0:
        df.to_csv("agentreview/llama_qwen_mistral/llama_run/df_eval_llama_agentreview_output.csv", index=False)
