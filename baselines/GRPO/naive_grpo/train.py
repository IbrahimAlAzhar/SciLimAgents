...
class GRPOConfig:
    ...
    output_dir: str = "GRPO/grpo_outputs"
...
# ============================================================
# 2. ZERO-SHOT REWARD MODEL (Rule-Based)
# ============================================================
...
SYSTEM_PROMPT = """You are a scientific peer reviewer. Given a research paper's text, identify and list the **limitations** of the study. For each limitation:
...
# ============================================================
# 5. ROLLOUT: Generate k responses per input
# ============================================================
...
def generate_rollouts(
    ...
    """
    For each sample, generate k responses at different temperatures.
    Disables gradient checkpointing during generation for compatibility.
    """
    model.eval()
    ...
    for sample in tqdm(samples, desc="Generating rollouts"):
        prompt = build_prompt(sample["input_text"], tokenizer)
        enc = tokenizer(
        ...
        for temp in config.temperatures[:config.num_rollouts]:
            out = model.generate(
                **enc,
...
# ============================================================
# 6. SCORING + ADVANTAGE COMPUTATION
# ============================================================
...
# ============================================================
# 7. GRPO POLICY UPDATE (LoRA-aware)
# ============================================================
...
def evaluate(
    ...
    """Evaluate policy on held-out data (greedy generation)."""
    model.eval()
...
# ============================================================
# 9. FULL TEST INFERENCE + PERFORMANCE REPORT
# ============================================================
...
def main():
    ...
    # We keep base_model as reference and wrap a copy with LoRA for policy.
    # But loading two copies is expensive. Instead:
    #   - ref_model = base_model (frozen, no LoRA)
    #   - policy_model = LoRA(base_model copy)
    # Since LoRA uses the same base, we load once and create LoRA on top.
