...
# ===========================================================================
# Text-only mode (fallback for smaller open-weight LLMs) ====================
# ===========================================================================
# Llama-3-8B / Mistral-7B / Qwen-2.5-3B can occasionally produce malformed
# JSON.  These prompts ask for a plain bulleted list instead.  We still keep
# the same NeurIPS reviewer framing so the methodology is comparable.

text_template_instructions = """
Respond with a plain bulleted list of the paper's WEAKNESSES and LIMITATIONS,
one item per line, each starting with "- ".  Do NOT add headings, JSON, or
any other formatting -- just the bullets.  Be specific and grounded in the
paper text.  Aim for between 3 and 8 bullets.
"""

text_reflection_prompt = """Round {current_round}/{num_reflections}.
Carefully reconsider the bulleted list of weaknesses you just produced.
Refine and improve it -- merge duplicates, fix vague items, add anything
important you missed.  Stick to the spirit of the original list unless there
are glaring issues.  Respond with the refined bulleted list ONLY, one item
per line, each starting with "- ".

If there is nothing to improve, repeat the previous list EXACTLY and add the
sentinel "I am done" on its own line at the very end.
DO NOT INCLUDE "I am done" IF YOU ARE MAKING CHANGES."""

def get_text_meta_review_prompt(reviews):
    """Meta-review prompt for text mode -- merge bullet lists into one list."""
    parts = [
        f"The following are bulleted weakness/limitation lists from "
        f"{len(reviews)} reviewers of the same paper.  Merge them into a "
        f"single de-duplicated bulleted list (each line starting with '- '). "
        f"Resolve disagreements conservatively and respect the consensus.\n"
    ]
    for i, r in enumerate(reviews):
        parts.append(f"\nReviewer {i + 1}/{len(reviews)}:\n{r}\n")
    parts.append("\nReturn ONLY the merged bulleted list, no preamble.")
    return "".join(parts)