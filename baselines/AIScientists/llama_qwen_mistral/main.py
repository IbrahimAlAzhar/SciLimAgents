...
def parse_args() -> argparse.Namespace:
    ...
    # ---- Output-mode / field controls ---------------------------------
    p.add_argument(
        "--mode",
        type=str,
        choices=["json", "text"],
        default="json",
        help="'json' (AI-Scientist default, full NeurIPS review JSON) or "
             "'text' (plain bulleted list -- safer for smaller open-weight LLMs).",
    )
    p.add_argument(
        "--limitation-field",
        type=str,
        default="Limitations",
        help="Which key to extract from the review JSON (json mode only). "
             "Choices: 'Limitations', 'Weaknesses', 'both'.  Use 'Weaknesses' "
             "if your ground truth is reviewer-written weaknesses.",
    )
    return p.parse_args()
    ...
    for idx in range(args.start, end):
        ...
        try:
            ...
            result = perform_limitation_review(
                ...
                mode=args.mode,
                limitation_field=args.limitation_field,
            )
            dt = time.time() - t_row
