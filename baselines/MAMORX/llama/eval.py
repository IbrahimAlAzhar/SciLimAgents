import pandas as pd 
df = pd.read_csv("baselines/MAMORX/llama/mamorx_limitations_llama_output.csv") 
...
def parse_merged_limitations(text_str):
    ...
    # Split the text by newline, optional spaces, digits, and a dot (e.g., '\n1. ', '\n2. ')
    chunks = re.split(r'(?:^|\n)\s*\d+\.\s+', text_str)
    ...
    for chunk in chunks:
        ...
        # Skip empty chunks or the introductory text (e.g., "Here is the synthesized list...")
        if not chunk or "synthesized list" in chunk.lower():
            continue
        
        # Extract the title and body
        # Expected chunk format: "**Unclear methodology**: The paper lacks..."
        title_match = re.match(r'\*\*(.*?)\*\*(.*)', chunk, re.DOTALL)
        
        if title_match:
            title = title_match.group(1).strip()
            body = title_match.group(2).strip()
            
            # Remove the leading colon and space that comes after the asterisks
            if body.startswith(':'):
                body = body[1:].strip()
            
            # Flatten any accidental line breaks in the description
            body = re.sub(r'\s*\n\s*', ' ', body)
            
            full_limitation = f"{title}: {body}"
        else:
            # Fallback if the ** formatting is completely missing
            full_limitation = re.sub(r'\s*\n\s*', ' ', chunk)
...
# =========================
# NEW: Exponential backoff with jitter for rate limit handling
# =========================
...
def evaluate_pairs_with_llm(pairs_list):
    ...
    for i, pair in enumerate(pairs_list):
        ...
        try:
            # ---- CHANGED: use retry wrapper instead of direct client call ----
...
for i, (index, row) in enumerate(tqdm(df.iterrows(), total=len(df), desc="Processing Rows")):
    ...
    time.sleep(10)
    if (i + 1) % 10 == 0:
        df.to_csv("baselines/MAMORX/llama/mamorx_eval_limitations_llama_output.csv", index=False)
