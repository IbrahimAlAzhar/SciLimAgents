...
def parse_merged_limitations(text_str):
    ...
    # INSTEAD of .splitlines(), we use regex to split by newlines ONLY when 
    # they are immediately followed by a Number (1.), a Header (#), or a Bullet (-, *).
    # This keeps multi-line paragraphs safely grouped together in a single 'block'.
    blocks = re.split(r'\n+(?=\d+\.|#|\*\*|-[ \t]|\*[ \t])', text_str.strip())
    ...
    for block in blocks:
        ...
        # 3. Check for Limitations using the double asterisk split with numbers
        if '**' in block and block[0].isdigit():
            parts = block.split('**')
            
            if len(parts) >= 3:
                title = parts[1].strip()
                # Join the rest, and flatten any internal newlines inside the block into spaces
        ...
        # 4. Fallback for standard bullet points (Handles both '-' and '*')
        ...
        # 5. NEW Fallback: Numbered item WITHOUT double asterisks
...
from openai import OpenAI, RateLimitError  # Added RateLimitError for retry logic
...
for i, (index, row) in enumerate(tqdm(df.iterrows(), total=len(df), desc="Processing Rows")):
    ...
    print("i is",i) 
    if (i + 1) % 10 == 0:
        df.to_csv("baselines/MAMORX/qwen/mamorx_eval_limitations_qwen_output.csv", index=False)
