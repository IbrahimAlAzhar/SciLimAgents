"""
MAMORX-adapted Baseline for Limitation Generation  (Qwen2.5-3B-Instruct version)
==================================================================================
Adapts the MAMORX multi-agent peer-review framework (Taechoyotin et al., 2024)
for limitation generation, running locally on Qwen2.5-3B-Instruct.
...
Run:
    python mamorx_limitation_generation_qwen.py \
        --input_csv  data/.../df_with_cited_in_processed.csv \
        --output_csv ./mamorx_limitations_qwen_output.csv \
        --max_papers -1
"""
...
log = logging.getLogger("mamorx-qwen")
...
log.info("Loading tokenizer and model: %s", MODEL_PATH)
...
def call_qwen(
    ...
    """Call Qwen2.5-Instruct via the HF pipeline, using the chat template."""
    messages = [
    ...
    # Qwen uses <|im_end|> as its end-of-turn token
...
def run_clarity_agent(paper_text: str) -> str:
    ...
    return call_qwen(CLARITY_AGENT_SYSTEM, user_msg)
...
def run_impact_agent(paper_text: str, retrieved_lit: str) -> str:
    ...
    return call_qwen(IMPACT_AGENT_SYSTEM, user_msg)
...
def run_experiment_agent(paper_text: str, cited_papers: str) -> str:
    ...
    return call_qwen(EXPERIMENT_AGENT_SYSTEM, user_msg)
...
def run_leader_agent(
    ...
    return call_qwen(LEADER_AGENT_SYSTEM, user_msg, temperature=0.2)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="MAMORX-style limitation generation (Qwen2.5 3B).")
    p.add_argument(
        "--input_csv",
        type=str,
        default=""
    ...
    p.add_argument(
        ...
        type=str,
        default=""
                "baselines/MAMORX/qwen/mamorx_limitations_qwen_output.csv",
    )
...
def main() -> None:
    ...
    # ------------------------------------------------------------------- #
    processed = 0
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="MAMORX-Qwen"):
