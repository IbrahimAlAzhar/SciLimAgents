"""
MAMORX-adapted Baseline for Limitation Generation  (Mistral-7B-Instruct-v0.3 version)
================================================================================
Adapts the MAMORX multi-agent peer-review framework (Taechoyotin et al., 2024)
for limitation generation, running locally on Mistral-7B-Instruct-v0.3.
...
Run:
    python mamorx_limitation_generation_mistral.py \
        --input_csv  data/.../df_with_cited_in_processed.csv \
        --output_csv ./mamorx_limitations_mistral_output.csv \
        --max_papers -1
"""
...
log = logging.getLogger("mamorx-mistral")
...
# Use eos as pad token if pad token is not set
...
def call_mistral(
    ...
    """Call Mistral-7B-Instruct via the HF pipeline, using the official chat template."""
    messages = [
...
def run_clarity_agent(paper_text: str) -> str:
    ...
    return call_mistral(CLARITY_AGENT_SYSTEM, user_msg)
...
def run_impact_agent(paper_text: str, retrieved_lit: str) -> str:
    ...
    return call_mistral(IMPACT_AGENT_SYSTEM, user_msg)
...
def run_experiment_agent(paper_text: str, cited_papers: str) -> str:
    ...
    return call_mistral(EXPERIMENT_AGENT_SYSTEM, user_msg)
...
def run_leader_agent(
    ...
    return call_mistral(LEADER_AGENT_SYSTEM, user_msg, temperature=0.2)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="MAMORX-style limitation generation (Mistral 7B v0.3).")
    p.add_argument(
        "--input_csv",
        type=str,
        default=""
    ...
    p.add_argument("--output_csv",        type=str, default="baselines/MAMORX/mistral/mamorx_limitations_mistral_output.csv")
...
def main() -> None:
    ...
    # ------------------------------------------------------------------- #
    processed = 0
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="MAMORX-Mistral"):
